...
@triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_constant_pad_nd_copy_new_zeros_permute_select_slice_view_8', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 3, 'num_store': 1, 'num_reduction': 0, 'backend_hash': '5E502224A319DB736ED388F470E3117A6892BC105B8AF0DAA4B752DFFD09C80F', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': True, 'min_split_scan_rblock': 256, 'spill_threshold': 32, 'store_cubin': False, 'deterministic': True, 'force_filter_reduction_configs': False, 'are_deterministic_algorithms_enabled': True, 'is_hip': True, 'has_loadstore_with_contiguous_rdim': False, 'tiling_scores': {'x': 44126208}, 'kernel_num_gb': 0.050429952, 'kernel_flop': 0},
    min_elem_per_thread=0
)
@triton.jit
def triton_poi_fused_constant_pad_nd_copy_new_zeros_permute_select_slice_view_8(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 6303744
    ...
    for r in tl.range(0, XBLOCK, R0_BLOCK, num_stages=2):
        ...
        tmp11 = tmp10 & tmp6
        tmp12 = tl.load(in_ptr0 + (512*((((656384 + x0 + 513*x2) // 512) % 513)) + 262144*((656384 + x0 + 513*x2) // 262656) + 786432*x1 + 786432*((656384 + x0 + 513*x2) // 787968) + (((x0 + 513*x2) % 512))), tmp11 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        ...
        tmp27 = tmp26 & tmp22
        tmp28 = tl.load(in_ptr0 + (262144*((((787712 + x0 + 513*x2) // 262656) % 3)) + 786432*((((787712 + x0 + 513*x2 + 787968*x1) // 787968) % 12)) + (((787712 + x0 + 513*x2) % 262656))), tmp27 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        ...
        tmp49 = tmp48 & tmp44
        tmp50 = tl.load(in_ptr0 + (262144*(((((-256) + x0 + 513*x2 + 262656*x3 + 787968*x1) // 262656) % 36)) + ((((-256) + x0 + 513*x2 + 262656*x3 + 787968*x1) % 262656))), tmp49 & xmask, other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr0 + (x4 + 6208*x3 + 24832*x2), tmp58, xmask)
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_constant_pad_nd_copy_new_zeros_permute_select_slice_view_8.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_constant_pad_nd_copy_new_zeros_permute_select_slice_view_8.benchmark_all_configs(*args)
