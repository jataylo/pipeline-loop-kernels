...
@triton.jit
def triton_poi_fused_new_zeros_0(out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 8192
...
def get_args():
    arg_0 = rand_strided((4, 1024, 2), (2048, 2, 1), device='cuda:0', dtype=torch.float16)
    return arg_0, 8192,
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_new_zeros_0.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_new_zeros_0.benchmark_all_configs(*args)
