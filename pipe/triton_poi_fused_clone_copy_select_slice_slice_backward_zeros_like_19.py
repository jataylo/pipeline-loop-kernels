...
@triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_clone_copy_select_slice_slice_backward_zeros_like_19', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 6, 'num_store': 1, 'num_reduction': 0, 'backend_hash': '5E502224A319DB736ED388F470E3117A6892BC105B8AF0DAA4B752DFFD09C80F', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': True, 'min_split_scan_rblock': 256, 'spill_threshold': 32, 'store_cubin': False, 'deterministic': True, 'force_filter_reduction_configs': False, 'are_deterministic_algorithms_enabled': True, 'is_hip': True, 'has_loadstore_with_contiguous_rdim': False, 'tiling_scores': {'x': 25214976}, 'kernel_num_gb': 0.018911232, 'kernel_flop': 0},
    min_elem_per_thread=0
)
@triton.jit
def triton_poi_fused_clone_copy_select_slice_slice_backward_zeros_like_19(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 3151872
    ...
    for r in tl.range(0, XBLOCK, R0_BLOCK, num_stages=2):
        ...
        tmp41 = tl.load(in_ptr0 + ((-1588992) + x4 + 6208*x2), tmp29 & xmask, other=0.0).to(tl.float32)
        tmp42 = tl.where(tmp35, tmp40, tmp41)
        tmp43 = tl.full(tmp42.shape, 0.0, tmp42.dtype)
        tmp44 = tl.where(tmp29, tmp42, tmp43)
        tmp45 = tl.load(in_ptr0 + ((-1588992) + x4 + 6208*x2), tmp13 & xmask, other=0.0).to(tl.float32)
        tmp46 = tl.where(tmp27, tmp44, tmp45)
        tmp47 = tl.load(in_ptr0 + (3178752 + x4 + 6208*x2), tmp13 & xmask, other=0.0).to(tl.float32)
        ...
        tmp70 = tl.load(in_ptr0 + ((-1588992) + x4 + 6208*x2), tmp58 & xmask, other=0.0).to(tl.float32)
        tmp71 = tl.where(tmp64, tmp69, tmp70)
        tmp72 = tl.full(tmp71.shape, 0.0, tmp71.dtype)
        tmp73 = tl.where(tmp58, tmp71, tmp72)
        tmp74 = tl.load(in_ptr0 + ((-1588992) + x4 + 6208*x2), tmp8 & xmask, other=0.0).to(tl.float32)
        tmp75 = tl.where(tmp56, tmp73, tmp74)
        tmp76 = tl.load(in_ptr0 + (3178752 + x4 + 6208*x2), tmp8 & xmask, other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr0 + (x0 + 513*x2 + 262656*x1), tmp86, xmask)
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_clone_copy_select_slice_slice_backward_zeros_like_19.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_clone_copy_select_slice_slice_backward_zeros_like_19.benchmark_all_configs(*args)
