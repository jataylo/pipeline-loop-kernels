...
@triton.jit
def triton_poi_fused_gelu_view_9(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 8388608
...
def get_args():
    ...
    arg_1 = rand_strided((4, 512, 4096), (2097152, 4096, 1), device='cuda:0', dtype=torch.float16)
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_gelu_view_9.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_gelu_view_9.benchmark_all_configs(*args)
