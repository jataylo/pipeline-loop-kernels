...
@triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_copy_select_slice_zeros_like_18', 'mutated_arg_names': [], 'optimize_mem': True, 'no_x_dim': False, 'num_load': 10, 'num_store': 1, 'num_reduction': 0, 'backend_hash': '5E502224A319DB736ED388F470E3117A6892BC105B8AF0DAA4B752DFFD09C80F', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': True, 'min_split_scan_rblock': 256, 'spill_threshold': 32, 'store_cubin': False, 'deterministic': True, 'force_filter_reduction_configs': False, 'are_deterministic_algorithms_enabled': True, 'is_hip': True, 'has_loadstore_with_contiguous_rdim': False, 'tiling_scores': {'x': 44126208}, 'kernel_num_gb': 0.025214976, 'kernel_flop': 0},
    min_elem_per_thread=0
)
@triton.jit
def triton_poi_fused_copy_select_slice_zeros_like_18(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 6303744
    ...
    for r in tl.range(0, XBLOCK, R0_BLOCK, num_stages=2):
        ...
        x4 = (xindex % 6156)
        x5 = xindex // 6156
        tmp72 = tl.load(in_ptr0 + (x4 + 6208*x2), xmask, eviction_policy='evict_last').to(tl.float32)
        tmp74 = tl.load(in_ptr0 + (4767744 + x4 + 6208*x2), xmask, eviction_policy='evict_last').to(tl.float32)
        tmp119 = tl.load(in_ptr0 + (x4 + 6208*x5), xmask).to(tl.float32)
        tmp0 = x3
        ...
        tmp41 = tl.load(in_ptr0 + (x4 + 6208*x2), tmp29 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp42 = tl.where(tmp35, tmp40, tmp41)
        tmp43 = tl.full(tmp42.shape, 0.0, tmp42.dtype)
        tmp44 = tl.where(tmp29, tmp42, tmp43)
        tmp45 = tl.load(in_ptr0 + (x4 + 6208*x2), tmp13 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp46 = tl.where(tmp27, tmp44, tmp45)
        tmp47 = tl.load(in_ptr0 + (4767744 + x4 + 6208*x2), tmp13 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        ...
        tmp68 = tl.load(in_ptr0 + (x4 + 6208*x2), tmp56 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        ...
        tmp107 = tl.load(in_ptr0 + (x4 + 6208*x2), tmp95 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp108 = tl.where(tmp101, tmp106, tmp107)
        tmp109 = tl.full(tmp108.shape, 0.0, tmp108.dtype)
        tmp110 = tl.where(tmp95, tmp108, tmp109)
        tmp111 = tl.load(in_ptr0 + (x4 + 6208*x2), tmp79 & xmask, eviction_policy='evict_last', other=0.0).to(tl.float32)
        tmp112 = tl.where(tmp93, tmp110, tmp111)
        tmp113 = tl.load(in_ptr0 + (x4 + 6208*x5), tmp79 & xmask, other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr0 + (x4 + 6208*x5), tmp122, xmask)


def get_args():
    arg_0 = rand_strided((12, 4, 256, 513), (513, 1589248, 6208, 1), device='cuda:0', dtype=torch.float16)
    arg_1 = rand_strided((12, 4, 256, 513), (513, 1589248, 6208, 1), device='cuda:0', dtype=torch.float16)
    return arg_0, arg_1, 6303744,
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_copy_select_slice_zeros_like_18.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_copy_select_slice_zeros_like_18.benchmark_all_configs(*args)
