...
@triton.jit
def triton_poi_fused_clone_transpose_view_19(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 2097152
    ...
    for r in tl.range(0, XBLOCK, R0_BLOCK, num_stages=2):
        ...
        x3 = xindex // 16384
        x4 = xindex
        tmp0 = tl.load(in_ptr0 + (x0 + 32*x2 + 4096*x1 + 16384*x3), None).to(tl.float32)
        tl.store(out_ptr0 + (x4), tmp0, None)


def get_args():
    arg_0 = rand_strided((512, 128, 32), (4096, 32, 1), device='cuda:0', dtype=torch.float16)
    arg_1 = rand_strided((128, 128, 4, 32), (16384, 128, 32, 1), device='cuda:0', dtype=torch.float16)
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_clone_transpose_view_19.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_clone_transpose_view_19.benchmark_all_configs(*args)
