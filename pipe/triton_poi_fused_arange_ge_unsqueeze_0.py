...
# kernel path: /tmp/hf_baseline/2v/c2vjms52rqgnlji4xfgkxcilhfcttd2xjkssmjteavcoihptb5ip.py
...
#   %unsqueeze : Tensor "i64[1, 1024][1024, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%iota, 0), kwargs = {})
#   %unsqueeze_1 : Tensor "i64[1, 1, 1024][1024, 1024, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%unsqueeze, 1), kwargs = {})
#   %unsqueeze_2 : Tensor "i64[1, 1, 1024, 1][1024, 1024, 1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%unsqueeze_1, 3), kwargs = {})
#   %ge : Tensor "b8[1, 1, 1024, 1][1024, 1024, 1, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.ge.Scalar](args = (%unsqueeze_2, 0), kwargs = {})
#   return %ge
...
def get_args():
    arg_0 = rand_strided((1, 1, 1024, 1), (1024, 1, 1, 1024), device='cuda:0', dtype=torch.bool)
    return arg_0, 1024,
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(0):
            torch.cuda.set_device(0)
            buf0 = empty_strided_cuda((1, 1, 1024, 1), (1024, 1, 1, 1024), torch.bool)
            # Topologically Sorted Source Nodes: [cache_position, q_indices, attention_mask], Original ATen: [aten.arange, aten.unsqueeze, aten.ge]
            stream0 = get_raw_stream(0)
            triton_poi_fused_arange_ge_unsqueeze_0.run(buf0, 1024, stream=stream0)
        return (reinterpret_tensor(buf0, (1, 1, 1024, 1024), (1024, 1024, 1, 0), 0), )
