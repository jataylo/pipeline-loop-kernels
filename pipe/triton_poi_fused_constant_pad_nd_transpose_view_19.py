...
@triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_constant_pad_nd_transpose_view_19', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 1, 'num_store': 1, 'num_reduction': 0, 'backend_hash': '5E502224A319DB736ED388F470E3117A6892BC105B8AF0DAA4B752DFFD09C80F', 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': False, 'max_autotune_pointwise': True, 'min_split_scan_rblock': 256, 'spill_threshold': 32, 'store_cubin': False, 'deterministic': True, 'force_filter_reduction_configs': False, 'are_deterministic_algorithms_enabled': True, 'is_hip': True, 'has_loadstore_with_contiguous_rdim': False, 'tiling_scores': {'x': 56770560}, 'kernel_num_gb': 0.031531008, 'kernel_flop': 0},
    min_elem_per_thread=0
)
@triton.jit
def triton_poi_fused_constant_pad_nd_transpose_view_19(in_ptr0, out_ptr0, xnumel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    xnumel = 9461760
    ...
    for r in tl.range(0, XBLOCK, R0_BLOCK, num_stages=2):
        ...
        tmp4 = tl.load(in_ptr0 + (x0 + 513*x2 + 6156*x1), tmp3, other=0.0).to(tl.float32)
        tl.store(out_ptr0 + (x3), tmp4, None)


def get_args():
    arg_0 = rand_strided((1, 1024, 12, 513), (6303744, 6156, 513, 1), device='cuda:0', dtype=torch.float16)
    arg_1 = rand_strided((12, 4, 256, 770), (788480, 197120, 770, 1), device='cuda:0', dtype=torch.float16)
    return arg_0, arg_1, 9461760,
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_constant_pad_nd_transpose_view_19.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_constant_pad_nd_transpose_view_19.benchmark_all_configs(*args)
