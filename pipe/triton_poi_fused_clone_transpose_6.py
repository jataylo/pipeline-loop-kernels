...
def get_args():
    arg_0 = rand_strided((1, 16, 256, 64), (262144, 64, 1024, 1), device='cuda:0', dtype=torch.float16)
    arg_1 = rand_strided((1, 256, 16, 64), (262144, 1024, 64, 1), device='cuda:0', dtype=torch.float16)
...
def call(args):
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_clone_transpose_6.run(*args, stream=stream0)


def benchmark_all_configs(args):
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0)
        return triton_poi_fused_clone_transpose_6.benchmark_all_configs(*args)
