...
@triton.jit
def triton_poi_fused__to_copy_nll_loss_forward_19(in_out_ptr0, in_ptr0, in_ptr1, out_ptr0, xnumel, XBLOCK : tl.constexpr):
    xnumel = 1
    ...
    tmp23 = tl.full([XBLOCK], 2, tl.int32)
    tmp24 = tmp22 + tmp23
    tmp25 = tmp22 < 0
    ...
    tmp62 = tmp52 + tmp61
    tmp63 = (tmp62 / tmp20)
    tl.store(out_ptr0 + (tl.full([XBLOCK], 0, tl.int32).broadcast_to(XBLOCK)), tmp20, None)
    tl.store(in_out_ptr0 + (tl.full([XBLOCK], 0, tl.int32).broadcast_to(XBLOCK)), tmp63, None)
